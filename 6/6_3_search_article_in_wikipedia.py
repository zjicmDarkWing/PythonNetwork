...
SEARCH_URL = "http://%s.wikipedia.org/w/api.php?action=auery&list=search&" \
             "srsearch=%s&sroffset=%d&srlimit=%d&format=yaml"
...
class Wikipedia:
    ...
    def search_content(self, query, page=1, limit=10):
        ...
        content = self._get_connect(url).read()
